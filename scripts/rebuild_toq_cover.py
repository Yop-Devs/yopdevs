"""Rebuild TOQ Tennis portfolio cover: logo + mosaic."""
from pathlib import Path
from PIL import Image, ImageDraw, ImageFilter, ImageEnhance

SAFE = Path(r"c:\Users\gabri\yopdevs\public\projetos\prints\toq\safe")
OUT = Path(r"c:\Users\gabri\yopdevs\public\projetos\prints\toq.png")
LOGO_SRC = Path(r"c:\Users\gabri\yopdevs\public\projetos\toq\logo.png")

logo = Image.open(LOGO_SRC).convert("RGBA")
print(f"logo -> {LOGO_SRC} {logo.size} mode={logo.mode}")

order = ["feed", "comunidade", "mensagens", "clube", "quadras", "torneios", "aulas"]
panels = [Image.open(SAFE / f"{k}.png").convert("RGB") for k in order]

W, H = 1920, 1080
canvas = Image.new("RGB", (W, H), (4, 8, 18))

glow = Image.new("RGB", (W, H), (4, 8, 18))
gd = ImageDraw.Draw(glow)
gd.ellipse((-140, 80, 700, 980), fill=(10, 40, 100))
gd.ellipse((980, -120, 2100, 680), fill=(20, 60, 140))
gd.ellipse((700, 600, 1500, 1200), fill=(30, 90, 180))
glow = glow.filter(ImageFilter.GaussianBlur(150))
canvas = Image.blend(canvas, glow, 0.48)

grid = Image.new("RGBA", (W, H), (0, 0, 0, 0))
gdraw = ImageDraw.Draw(grid)
for x in range(0, W, 48):
    gdraw.line([(x, 0), (x, H)], fill=(70, 130, 220, 14))
for y in range(0, H, 48):
    gdraw.line([(0, y), (W, y)], fill=(70, 130, 220, 14))
...
bp.rounded_rectangle((0, 0, brand_w - 1, 879), radius=28, fill=(6, 12, 28, 245))
bp.rounded_rectangle((0, 0, brand_w - 1, 879), radius=28, outline=(60, 130, 230, 140), width=2)
...
bp.text((brand_w // 2, 790), "TOQ TENNIS", fill=(220, 235, 255, 230), anchor="mm")
bp.text((brand_w // 2, 828), "Rede social & gestão de quadras", fill=(140, 170, 210, 180), anchor="mm")
