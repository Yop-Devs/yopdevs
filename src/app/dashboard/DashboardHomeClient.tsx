...
import { formatTimeShort, parseLocalYMD, toLocalYMD } from '@/lib/agenda'

const ONLINE_MS = 5 * 60 * 1000

type AgendaRemindRow = {
  id: string
  title: string
...
export default function DashboardHomeClient() {
  ...
  const [agendaReminders, setAgendaReminders] = useState<AgendaRemindRow[]>([])
  ...
  async function loadData() {
    try {
      ...
      const [postsRes, projsRes, projsTodayRes, profilesRes, commentsRes, agendaRes] = await Promise.all([
        ...
        supabase
          ...
          .select('id, title, description, activity_date, activity_time')
          .eq('user_id', user.id)
          .lte('activity_date', todayYmd),
      ])
      ...
      const byAgendaTime = (a: AgendaRemindRow, b: AgendaRemindRow) => {
      ...
      if (!agendaRes.error && agendaRes.data?.length) {
        const raw = agendaRes.data as AgendaRemindRow[]
        const todayItems = raw.filter((r) => r.activity_date === todayYmd)
        const overdue = raw.filter((r) => r.activity_date < todayYmd)
        todayItems.sort(byAgendaTime)
        overdue.sort((a, b) => {
          const c = a.activity_date.localeCompare(b.activity_date)
          return c !== 0 ? c : byAgendaTime(a, b)
        })
        setAgendaReminders([...todayItems, ...overdue])
      } else {
        setAgendaReminders([])
      }
      const dismissKey = `yop_agenda_remind_${user.id}_${todayYmd}`
      if (typeof window !== 'undefined') {
        setAgendaBannerDismissed(localStorage.getItem(dismissKey) === '1')
      } else {
  ...
  const handleDismissAgendaBanner = useCallback(() => {
    void supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user || typeof window === 'undefined') return
      const key = `yop_agenda_remind_${user.id}_${toLocalYMD(new Date())}`
      localStorage.setItem(key, '1')
      setAgendaBannerDismissed(true)
    })
  }, [])
  ...
  const todayYmdLabel = toLocalYMD(new Date())
  ...
  return (
    <div className="max-w-[1400px] mx-auto w-full min-w-0 px-4 sm:px-6 py-4 sm:py-6 md:py-10 space-y-4 sm:space-y-6 md:space-y-10">
      ...
      {!agendaBannerDismissed && agendaReminders.length > 0 && (
        <div className="rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-50 to-white p-4 sm:p-5 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="min-w-0">
              <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2">
                Lembretes da sua agenda
              </p>
              <ul className="space-y-2">
                {agendaReminders.map((r) => (
                  <li key={r.id} className="text-sm text-slate-800">
                    ...
                    {r.activity_time ? (
                      <span className="text-indigo-600 font-semibold ml-2">{formatTimeShort(r.activity_time)}</span>
                    ) : null}
                    <span className="text-slate-500 text-xs ml-2">
                      {r.activity_date === todayYmdLabel
                        ? 'hoje'
                        : `desde ${parseLocalYMD(r.activity_date).toLocaleDateString('pt-BR', {
                            day: 'numeric',
                            month: 'short',
                          })}`}
                    </span>
            ...
            <div className="flex flex-col gap-2 shrink-0 sm:items-end">
              <Link
                ...
                className="inline-flex justify-center sm:justify-end text-center text-[10px] font-black uppercase tracking-wider text-white bg-indigo-600 px-4 py-2 rounded-lg hover:bg-indigo-700"
              >
                Abrir agenda
              </Link>
              <button
                type="button"
                onClick={handleDismissAgendaBanner}
            ...
            {activityItems.length === 0 ? (
              ...
              <div className="space-y-5 sm:space-y-6">
                {activityItems.map((item) => {
                  ...
                  const postDisplay = isPost ? forumPostHeadlineAndBody(d.title, d.content) : null
                  const preview = isPost && postDisplay
                    ? (postDisplay.body
                      ? String(postDisplay.body).replace(/\n/g, ' ').slice(0, 120) + (postDisplay.body.length > 120 ? '...' : '')
                      : null)
                      ...
                      <Link href={href} className="block">
                        <div className="flex gap-4">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-semibold text-slate-900 truncate group-hover:text-violet-800 transition-colors">
                              {profileData?.full_name || 'Anônimo'}
                            </p>
                            <p className="text-xs text-slate-500 mt-0.5 mb-2">
                              {isPost ? 'Publicação no fórum' : 'Oportunidade'} · {formatTimeAgo(new Date(d.created_at))}
                            </p>
                            {isPost ? (
                              postDisplay?.showHeadline ? (
                                <>
                                  <p className="text-sm font-semibold text-slate-900 leading-snug">{postDisplay.headline}</p>
                                  {preview ? (
                                    <p className="text-sm text-slate-700 font-normal leading-relaxed mt-1.5 line-clamp-3">{preview}</p>
                                  ) : null}
                                </>
                              ) : preview ? (
                                <p className="text-sm text-slate-800 font-normal leading-relaxed line-clamp-4">{preview}</p>
                              ) : null
                            ) : (
                              <>
                                <p className="text-sm font-semibold text-slate-900 leading-snug">{d.title}</p>
                                {preview ? (
                                  <p className="text-sm text-slate-600 font-normal leading-relaxed mt-1.5 line-clamp-2">{preview}</p>
                                ) : null}
                              </>
                            )}
