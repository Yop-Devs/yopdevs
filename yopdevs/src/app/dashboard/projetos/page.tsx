      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {filtered.map((project) => (
          <div key={project.id} className="bg-white border-2 border-slate-900 rounded-2xl p-8 hover:shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transition-all flex flex-col justify-between h-full group">
            ...
            <div className="flex items-center justify-between border-t-2 border-slate-50 pt-8">
              <Link href={`/dashboard/perfil/${project.owner_id}`} className="flex items-center gap-3 hover:opacity-80">
                <div className="w-10 h-10 bg-slate-100 rounded-xl border-2 border-slate-900 overflow-hidden flex items-center justify-center font-black">
                  {project.profiles?.avatar_url ? <img src={project.profiles.avatar_url} className="w-full h-full object-cover" /> : project.profiles?.full_name?.[0]}
                </div>
                <div>
              ...
              <Link href={`/dashboard/chat/${project.owner_id}`} className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">
                Conectar ↗
              </Link>
            </div>
          </div>
